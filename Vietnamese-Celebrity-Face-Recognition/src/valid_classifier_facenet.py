...
def convert2categorical(labels):
    ...
    for index, row in labels.iterrows():
        categories.append(to_categorical(row[1], num_classes = NUM_CLASSES+1))
...
if __name__ == '__main__':
    ...
    model = load_model('output/output_facenet_adacos_KEEP_ADACOSLAYER/best_model')

    labels_test = pd.read_csv('processed_data\embedded_face_val\embs_class_20180402-114759.csv')
    labels_test.columns = ['emb', 'class']
    categories_test = convert2categorical(labels_test)

    labels_test.reset_index(inplace = True)
    test_generator = DataGenerator(labels_test, categories_test, BATCH_SIZE)

    overall_score = model.evaluate(test_generator,
                        verbose = 'auto',
                        workers=1,
                        use_multiprocessing=False)
