...
def build_model():
    model_for_preds = Sequential()
    model_for_preds.add(Dense(2048, input_dim = INPUT_SHAPE, activation = "relu"))
    model_for_preds.add(BatchNormalization())
    model_for_preds.add(Dropout(0.25))
    model_for_preds.add(Dense(NUM_CLASSES+1, activation = "relu"))
    model_for_preds.add(BatchNormalization())

    onehot_labels = Input(shape=(NUM_CLASSES+1,), dtype='int32', name='onehot_label')
    predictions = AdaCos(n_classes=NUM_CLASSES+1)([model_for_preds.output, onehot_labels])

    last_model = Model(inputs=[model_for_preds.input, onehot_labels], outputs=predictions)

    return last_model, model_for_preds
...
if __name__ == '__main__':
    model, model_for_preds = build_model()
